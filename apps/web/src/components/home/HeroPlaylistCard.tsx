...
export function HeroPlaylistCard({
  ...
  return (
    <motion.div
      ...
      className='relative group shrink-0 w-[82vw] max-w-[320px] md:max-w-none md:w-[500px] h-[160px] md:h-[220px] rounded-[1.75rem] md:rounded-[2.25rem] overflow-hidden border border-white/10 transition-all duration-500 bg-zinc-900/30 dark:bg-black/20 backdrop-blur-xl snap-start cursor-pointer shadow-2xl block'
    ...
    >
      ...
      <div className='absolute inset-0 p-4 md:p-6 flex gap-4 md:gap-6 items-center z-10'>
        ...
        <div className='relative shrink-0 w-24 h-24 md:w-[172px] md:h-[172px] rounded-xl md:rounded-[1.5rem] overflow-hidden shadow-2xl border border-white/10 group-hover:scale-[1.02] transition-transform duration-500 ease-out'>
          {playlist.images?.[0]?.url ? (
            <Image
              src={playlist.images[0].url}
              fill
              sizes='(max-width: 768px) 96px, 172px'
              className='object-cover'
              alt={playlist.name || ''}
              priority={index < 2}
            />
        ...
        <div className='flex-1 flex flex-col justify-between h-full min-w-0 py-0.5 md:py-1'>
          {/* Top part: Title */}
          <div className='min-w-0'>
            <h2 className='text-sm md:text-xl font-black text-white mt-0.5 md:mt-1 leading-tight tracking-tight drop-shadow-md group-hover:text-primary transition-colors line-clamp-2'>
              {playlist.name || 'Untitled Playlist'}
            </h2>
            <p className='text-zinc-300 text-[9px] md:text-xs line-clamp-2 md:line-clamp-3 mt-1 md:mt-2 font-medium leading-relaxed'>
              {decodedDescription ||
          ...
          <div className='flex items-center justify-between gap-2 mt-1 md:mt-2 mb-0.5'>
            <div className='flex items-center gap-1.5 md:gap-2 text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-zinc-400 min-w-0 font-outfit'>
              ...
              {playlist.owner?.display_name && (
                <>
                  ...
                  <span className='truncate max-w-[80px] md:max-w-[130px] font-outfit'>
